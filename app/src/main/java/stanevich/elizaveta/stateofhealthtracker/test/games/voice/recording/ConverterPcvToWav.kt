...
import com.google.android.gms.common.internal.safeparcel.SafeParcelWriter.writeShort
import java.io.*
...
class ConverterPcmToWav {
    ...
    fun pcmToWave(pcmFile: File, waveFile: File) {
        ...
        header[23] = 0
        // sample rate
        header[24] = (sampleRate and 0xff) as Byte
        header[25] = (sampleRate shr 8 and 0xff) as Byte
        header[26] = (sampleRate shr 16 and 0xff) as Byte
        header[27] = (sampleRate shr 24 and 0xff) as Byte
        // bit rate
        var bitrate = sampleRate * channel * format
        header[28] = (bitrate / 8 and 0xff) as Byte
        header[29] = (bitrate / 8 shr 8 and 0xff) as Byte
        header[30] = (bitrate / 8 shr 16 and 0xff) as Byte
        header[31] = (bitrate / 8 shr 24 and 0xff) as Byte
        // block align
        header[32] = ((channel * format) / 8) as Byte
        header[33] = 0
        // bits sample
        header[34] = 16
        header[35] = 0
        ...
        header[39] = 'a'.toByte()
        // subchunk 2 size
        header[40] = (data.size and 0xff) as Byte
        header[41] = (data.size shr 8 and 0xff) as Byte
        header[42] = (data.size shr 16 and 0xff) as Byte
        header[43] = (data.size shr 24 and 0xff) as Byte
